...
export default function Gallery(){
    const photos = [photo1,photo2,photo3,photo4,photo5,photo6,photo7,photo8,photo9,photo10,photo11,photo12,photo13]
    return(
        <div className="flex flex-col items-center mb-6 ">
            <h3 className="text-2xl text-yellow-600 md:text-4xl font-semibold uppercase mt-5">Gallery</h3>
            <div class="w-[8.125rem] h-1 border-t-4 border-cyan-700 mt-2 mb-10"></div>
            <div className="flex flex-row space-x-2 flex-nowrap whitespace-nowrap w-full overflow-x-scroll scrollbar-hidden">
                {photos.map((photo,index) => (
                    <img key={index} alt={`Photo ${index + 1}`}src={photo} className=" h-72 w-auto"/>
                ))}
            </div>
        </div>
    )
